...
- **Request Distribution**: Arrival process (Poisson burst vs. fixed concurrency) and length distributions (prompt length N_in, generation length N_out).
...
- **Sample Size**: Minimum N >= 30 iterations for statistical significance.
...
3. **Derive Theoretical Roofline Metrics**:
   - `Arithmetic Intensity I = Total FLOPs / Total Memory Bytes Transferred`
   - `Attainable Performance P = min(Peak Compute FLOP/s, I * Peak Memory Bandwidth Byte/s)`
